...
- **The orchestrator is a CEO, not an IC. Route work; don't do it.** (Sharpened 2026-04-27 after repeated violations.) The orchestrator's job is to dispatch teammates, set quality bars, route critiques, retire members — not to read PNGs, edit code, run smoke, or grade renders. Even when the work looks like it would take 30 seconds and routing through a teammate looks like overhead, **always route**. The orchestrator-as-IC failure mode produces three losses: (1) the team-channel context disappears (teammates don't know what was changed), (2) the iteration loop's separation-of-concerns collapses (impl and critic were specifically split to prevent author bias from contaminating grading), (3) the orchestrator burns context and gets blind to what's actually happening. Concretely: if the next instinct is "I'll just X" — STOP and send it to the right teammate instead. If the right teammate doesn't exist, spawn one; if they're broken, respawn them; if the team-channel routing is broken, fix the routing. Never substitute orchestrator-direct work for missing/broken team mechanics — that's the workaround-vs-theater rule (TEAMS.md §3) applied to coordination.
- **Teammates for high-level work; sub-agents are an internal tool.** Established 2026-04-25. When dispatching feature implementation, audits, reviews, research streams that may need follow-up, or anything that should retain context across a multi-turn iteration: use **named teammates** (first call `TeamCreate({team_name, ...})` to create the team, then spawn members with `Agent({team_name, name, ...})` so they inherit the team channel and `SendMessage` works — both halves are required; the `name` alone is not enough) — the structure documented in `.claude/TEAMS.md`. Cog Reviewer is the same Cog Reviewer across the Section Timer audit, the Whiteboard pipeline audit, and the Whiteboard pipeline-v2 re-audit; PM Interpreter is the same PM across all the specs. One-shot sub-agents (the `Agent` tool with no `name`) are something a teammate may use *internally* to parallelise their own work — that is the teammate's call, not the orchestrator's. **The rule for the orchestrator (this conversation): every high-level implementation, audit, review, or research dispatch goes to a teammate.** A useful side-effect is that "the verifier was too aggressive — tune it" can `SendMessage` the original verifier instead of re-briefing a fresh agent and losing nuance.
- **Never engineer around failure — diagnose it.** (CORE principle, established 2026-04-27, sibling to **Isolation and investigation**.) When something fails — a smoke test stalls, an API call rate-limits, a render comes back wrong — the wrong instinct is to *engineer through it*: add retry logic, exponential backoff, fallbacks, defensive bandages, "robustness" layers. Every retry hides signal; every fallback rationalises around the broken thing instead of confronting it. Real fix work is the opposite: **sit down, isolate, root-cause.** Concrete consequences:
  - **No retry logic.** If something fails once, that's a signal worth reading, not a transient glitch to retry past. If it's truly transient, it's the platform's job, not ours. The only legitimate retry I will write is one the underlying platform's contract explicitly requires (e.g. fetching with a documented `Retry-After` header against a documented 429 ceiling) — not speculative defense against unknown intermittency. Round-12b shipped 9-attempt retry with exponential backoff against a rate-limited API; that 30+60+120+240+480+480+480+480 = 40 minutes of sleeping doing zero useful work. The right answer was "stop after attempt 2 same-failure, root-cause why we're being throttled."
  - **No fallbacks for things that should work.** A fallback is "this code path failed but here's a worse one we'll quietly take." Pre-empt the failure or fix the path; don't half-fail silently. The whiteboard fallbacks added in earlier rounds (clear_scene + retry, partial-scene-on-error persistence, "if X then Y else log-and-continue") obscured the real problems for two cycles before we admitted the architecture was wrong.
  - **No "robustness" layers around symptoms.** When the round-11 in-MCP critic emitted bogus `wrapper_geometry` defects, my round-12a instinct was to add 4 wrapper rejections + a "must-patch" agent rule + AND a critic-prompt fix on top. That was three layers of defense around a 5-line rasteriser bug. The rasteriser fix made the other three layers possibly unnecessary (TBD post-smoke). One root-cause fix dominates three symptom-layer patches.
  - **The discipline: STOP, isolate, root-cause.** When something fails, sit with it. Build a small reproduction that doesn't require the full pipeline. (See **Isolation and investigation** below.) Look at the failure mode honestly — what is it telling me? What's the *one* thing that would have to be true for this to work? Only after the root cause is identified and named do I edit code. If the root cause turns out to be external (rate-limit, account quota, OS-level), the fix is operational (wait, switch account, scale down concurrency), not architectural.
  - **The forcing test: would I write this code if the underlying thing worked?** If no, I'm engineering around failure. Delete and start over. Examples: "would I have written this 9-attempt retry if the API never throttled? No → it's symptom-layer." "Would I have added `try/catch + log + continue` here if errors were impossible? No → it's hiding signal."
- **Isolation and investigation** (CORE principle, established 2026-04-25). When something is broken in a complex pipeline, **isolate the broken part** and iterate on it alone. Don't repeatedly rebuild the whole app, regenerate expensive AI outputs, and re-test end-to-end when the bug is in one downstream stage. For the whiteboard: if the rendered diagram is wrong but Pass 1 + Pass 2 outputs are saved on disk in the per-paper sidecar, the bug is in the render layer (ELK layout + convertToExcalidrawElements + exportToCanvas). Build a CLI / module entry point that takes a saved WBDiagram JSON and produces a PNG, with NO Claude calls. Iterate on the render until the PNG is clean — fast feedback, no API spend per iteration. Then plug the fixed render module back into the live pipeline.
  - The same shape applies anywhere: a multi-stage AI pipeline whose output is wrong should be debugged by isolating each stage. Save each stage's output to disk; build per-stage CLIs that consume one stage's output and produce the next; iterate on the broken stage in isolation; integrate at the end.
  - Anti-pattern this prevents: "the diagram is bad → rebuild app → regenerate ($1.90) → look → still bad → tweak ELK constant → rebuild → regenerate ($1.90) → look → still bad → ..." Five iterations like that is $9.50 of API spend and an hour of build cycles, all to debug a 50-line layout function. Isolating the render takes 30 seconds per iteration and zero API spend.
  - This composes with the close-the-loop principle: close-the-loop says "look at the output before declaring done"; isolation says "when looking reveals a bug, isolate the broken stage and iterate there." The two together are the development model.
- **AI agents close their own visual loop too** (established 2026-04-25). The close-the-loop principle below applies to the *implementer team* (Claude Code agents shipping code). It also applies to **AI agents inside the product** that generate visual artefacts (diagrams, images, layouts). When the whiteboard pipeline generates a WBDiagram, the AI that wrote the JSON cannot trust that JSON until it has *seen* the rendered output and judged it against the spec. Concretely for the whiteboard:
  - After Pass 2 emits a WBDiagram, render it through ELK + Excalidraw → export to PNG/SVG.
  - Feed that image back into a vision-capable Claude call with the same Pass 2 prompt + the spec's quality rules.
  - Claude critiques: *"text overflows in node L1.3; arrow L1.2→L1.3 crosses node L1.4; figure embed didn't resolve."* Emits a patch (or a fresh diagram).
  - Re-render. Re-look. Iterate up to N rounds (suggest 3 — past that, the model is likely thrashing rather than improving).
  - Only then return the diagram to the user-facing renderer.
  This is the same principle as the human implementer's close-the-loop: shipped output must be looked at by the producer before being shown. Generation-without-looking is a process failure regardless of whether the producer is a human teammate or an in-product Claude call.
- **Team-orchestration rules live in `.claude/TEAMS.md`** (established 2026-04-26). All standing rules for how the orchestrator runs the team — visual-artefact iteration loops, critics-as-user-proxies, signal-equivalence vs theater, team hygiene (≤6 active members), standup polling, **implementer discipline (§6) and QA verification discipline (§7) added 2026-04-28** — are documented in `.claude/TEAMS.md` "Workflow rules" section. Read that file when dispatching teammates, spawning critics, retiring members, or making the workaround-vs-fix call. CLAUDE.md is the principles index; TEAMS.md is the operational playbook for the agent harness. (Critic rubrics live separately at `.claude/critics/<name>.md`; that file is the durable home for any standing rubric a critic teammate must inherit at spawn.)
- **Impl teammates diagnose, then STOP — no edits without explicit task assignment, no mutation of user state ever** (CORE, established 2026-04-28, full text in TEAMS.md §6). Investigation dispatches return a structured report; the orchestrator decides whether and where to apply a fix. The user's live `lens.db`, sidecars, logs, and PDF library are read-only from impl teammates. A single edit-before-task-assigned event triggers retirement.
- **QA teammates verify the FINAL USER-VISIBLE PRODUCT, headless, never on the user's screen** (CORE, established 2026-04-28, full text in TEAMS.md §7, user verbatim: *"QA's main purpose is not to check the logic; it's to check whether the final product is working or not. And they should ideally do that in a headless manner or by not disrupting my current screen, because I might be on another window as well doing something. They can use AppleScript or whatever they want."*). Acceptable QA = launch an isolated Fathom against `/tmp/fathom-test-<hash>` user-data dir, drive via AppleScript / Electron remote debugging / Playwright-electron, position off-screen or hide. Unacceptable QA = grading source-code logic without driving a running instance, taking focus on the user's screen, mutating the user's live state. Source-code claims do NOT close bugs; observed user-visible behavior does.
- **Spawned teammates need the team channel, not just a `name`** (lesson learned 2026-04-26). When using the `Agent` tool to spawn a teammate, passing only `name` is insufficient — the resulting agent gets a `general-purpose` sub-agent toolset that does NOT include `SendMessage`, so they can't communicate back to the orchestrator or each other. Symptom: spawned agent reports back "No SendMessage tool registered in this session" or simply goes idle without delivering work. Fix: the orchestrator must first call `TeamCreate({team_name, ...})` to create a team, then spawn members with `Agent({team_name, name, ...})` so they inherit the team channel. Without `team_name`, named-Agent spawns are functionally one-shot sub-agents despite the name. If `TeamCreate` is itself unavailable in the current environment, fall back to doing the work directly in the orchestrator conversation rather than running iterations of unspawnable critics.
- **Standing approval for routine in-product Claude spend** (established 2026-04-25). The user has explicitly granted standing approval for the whiteboard generation pipeline (~$1.90/paper one-time + ~$0.005/iteration). Sub-agents that produce visual artefacts (whiteboard's Pass 1 + Pass 2 + the new visual-critique loop) do not require per-spend confirmation. The user said: *"You have my rebuild approval. You have the spend approval. You have all these approvals; you don't even need to see the spend approval. Don't even ask for it from the user."* This standing approval covers the whiteboard pipeline specifically; *new* AI features that introduce a different cost profile (e.g. cross-paper queries, video generation, model upgrades that change pricing) still require a fresh consent prompt before first use.
...
- **Whiteboards persist by default.** (Established 2026-04-27, user verbatim: *"Once I have generated the whiteboard, the whiteboard should be there even the next time when I open it. I shouldn't have to remake it. We should save the whiteboard, and unless the user clears the whiteboard, we should not delete that whiteboard. We should see that the next time I open it or close it."*) Same principle as zoom-persists, applied to the whiteboard surface. Once the user has paid the API cost (~$1.90/paper) to generate a whiteboard, regenerating it because we forgot to save is a design failure. The whiteboard scene + any chat-refinement state lives next to the PDF (per §9 sidecar pattern); on reopen, hydrate from disk first, regenerate only if no saved state exists. The ONLY paths to whiteboard deletion are: (a) the user explicitly clicks "Clear whiteboard" / "Regenerate"; (b) the PDF itself is removed from the user's library. Anything else is a bug.
...
- **Component-as-answer framing — every component connects to the ground problem** (established 2026-04-27, user verbatim: *"when we are listing the modules or different components, it might help to understand things in a way that asks what is the answer that each component is answering. For example… cross-attention to dyno v3 patches helps us answer: what does the 3D point look like in each photo? Sparse self-attention can help answer in this specific problem… how does this point relate to its neighbors? Which view should I trust here? … the user is very much focused and oriented towards how everything connects to the ground problem rather than just how these details are interconnected."*). When an explanation lists modules, components, equations, or sub-systems — **in lens text, in whiteboard nodes, in any surface where Claude names a piece of the paper** — each piece must be framed as the **answer to a specific question** that traces back to the **ground problem the paper is solving**. Wrong: "cross-attention to DINOv3 patches" presented as a standalone component. Right: "cross-attention to DINOv3 patches → answers: what does this 3D point look like in each photo?" The user's mental model is grounded in the paper's end goal; component-to-component interconnection without ground-problem terminus reads as detail-soup. Concrete consequences:
  - **Lens explainer prompt** must instruct Claude to identify the paper's ground problem (from the digest) before naming any component, and to state the question each named component answers about that ground problem.
  - **Whiteboard PASS2_SYSTEM** must require every node label to carry a "→ what question does this node answer?" framing — the question becomes the node subtitle or annotation, not optional decoration.
  - **Whiteboard critic rubric** grades component framing — components without question-as-answer framing are ITERATE; questions that connect components only to other components instead of to the ground problem are also ITERATE.
  - **Equation explanations** (already required in depth per round-3 rubric) must additionally frame the equation as the answer to a problem-level question — what does this loss / update rule / sampling step let us do that gets us closer to the paper's end goal?
...
- **The in-product agent must close its own visual loop with per-stage isolation — and a critic APPROVED is not a substitute** (CORE principle, sharpened 2026-04-27 after critic-missed-defects incident, user verbatim: *"These all things should have been caught by the AI agent when it sees the screenshot. The AI agent that is actually generating the whiteboard isolates the different steps in the whiteboarding process and sees which one is performing wrongly. This should be one of your methods for building the product and the isolating principle that I have already mentioned and that is already there in your instructions or document. One of these isolating principles is also to isolate within the isolated components to see which individual part is performing wrongly"*). Two principles already in this file ("Isolation and investigation" §0 and "AI agents close their own visual loop too" §0) compose into a binding rule for the in-product whiteboard agent: after rendering, the agent must *itself* look at the rendered PNG, attribute any visible defect to the pipeline stage that produced it (Pass 1 narrative? Pass A planning? Pass B placement? wrapper geometry? renderer/ELK layout?), isolate within that stage to the failing sub-component, fix it, and re-render. Concrete consequences:
  - **Pass 2.5 visual self-critique is mandatory infrastructure, not a planned-someday feature.** The current pipeline has NO in-product visual loop — it generates Pass 1 + Pass 2, renders, ships. Any defect surviving to the user is one the in-product agent never looked at. Building the IPC + critique call is the highest-priority harness work; tracked at todo #58 (path A) and now blocking other work.
  - **Per-stage isolation tooling is half-built and must be finished.** CLI entry points exist for some stages (`scripts/wb-render-current.mts`); explicit per-stage attribution + a Pass 2.5 hook that runs after render do not. When the in-product agent finds a defect at stage K, the harness must support re-running ONLY stage K with a fix, not regenerating the entire $1.90/paper Pass 1+2 chain.
  - **Critic APPROVED is necessary but not sufficient.** The standing critic is a *user proxy*; their grade is best-effort but can miss defects the user catches at first glance. When the user reports a defect on a critic-APPROVED render, the failure is at *two* layers: the critic missed it AND the in-product visual loop didn't exist (or didn't catch it) before the critic ever saw it. Per the existing "reported-failure retrospection" rule (§0): retrospect honestly, *add instrumentation*, capture the new pattern, then make the harness detect it next time. Critic-misses are always treated as a critic-rubric expansion AND a missing-instrumentation gap, never just "the critic was wrong; we'll be more careful."
  - **The agent harness exists to make this class of failure rarer over time, not faster to patch when it happens.** Reshape the harness — Pass 2.5 + per-stage isolation + tool-layer rejection (next bullet) — and iterate the whiteboard outputs through *that* infrastructure. Not the other way around.
- **Tools enforce constraints; prompts only guide intent — for any structural defect, the fix belongs in the tool, not in the prompt** (CORE principle, established 2026-04-27, user verbatim: *"We should have structural ways to prevent this, or tools can just simply do the computation and let the tool know when it's going to get out of the box or when something's going to overlap, so that these kinds of problems don't happen. We need to fundamentally think, rather than just improving the prompt, on how we can make better tools and … better agent harnesses for the agent to make the whiteboard and address the concerns that we see. This should be part of one of our core philosophies and the way we work."*). The class of defects this targets is structural: text that overflows a container, elements that overlap each other, lines that cross labelled regions, anything where a fast geometric check at tool-call time would have caught the impossibility before the agent emitted it. The wrong move when these defects appear is to add another paragraph of prompt instructions ("text must fit," "labels must not overlap"). The right move is to extend the tool: compute the bbox / wrap / collision check inside the tool wrapper, and when the agent's call would produce an invalid artefact, *return an error message that tells the agent which constraint failed and what would satisfy it.* The tool's error surface becomes the forcing function — the agent literally cannot ship the bad output through that tool. Concrete consequences:
  - **AC predicates are fallback, not first line.** ACs catch what the tool layer missed. If a defect class shows up in an AC fail, the next round's work is to push that check into the tool wrapper so the AC becomes redundant for that class. ACs persist as belt-and-suspenders, but the tool is the belt.
  - **Tool wrappers compute, then accept/reject.** `create_callout_box` already does width-aware wrap (round 7); the next step is to refuse the call if the author-supplied callout dimensions can't hold the wrapped text, with a precise error: "callout body wraps to 5 lines at 320px width × 24px line-height = 120px content height, but callout is 80px tall — increase height to ≥160px or shorten text." `create_arrow_with_label` (or whatever the equivalent is) must compute the label's predicted bbox and check against every existing element bbox; if the label collides, the tool either auto-positions to a free spot OR returns "label collides with element X at (x,y); pass `labelOffset: {dx, dy}` to position elsewhere or shorten the label."
  - **The principle composes with AI-built-product (§1) and close-the-loop (§1).** Tools that compute-then-reject are how the AI implementer "looks at" its own output before shipping — the look happens inside every tool call, not as a separate after-the-fact pass. The visual self-loop survives as the highest-level check, but most defects never reach it.
  - **Critic recommendations are graded on this axis too.** When the critic identifies a structural defect, the recommended fix MUST target the tool layer, not the prompt. A pipeline-modification ask of the shape "PASS2_SYSTEM should add a rule that text must fit" is a weaker ask than "MCP wrapper should reject `create_callout_box` calls where the wrapped text height exceeds the box's inner height." The critic rubric grades the *kind* of recommendation as well as its substance.
  - **This is the agent harness as a first-class artefact (§0) applied to the in-product pipeline.** We're not building only a PDF reader; we're building the harness — tools, ACs, prompts, critics — that produces good explanations on demand. Every round of iteration that improves the harness compounds; every round that fixes one artefact via prompt nudging is local to that artefact.
