...
- **The in-product agent must close its own visual loop with per-stage isolation — and a critic APPROVED is not a substitute** (CORE principle, sharpened 2026-04-27 after critic-missed-defects incident, user verbatim: *"These all things should have been caught by the AI agent when it sees the screenshot. The AI agent that is actually generating the whiteboard isolates the different steps in the whiteboarding process and sees which one is performing wrongly. This should be one of your methods for building the product and the isolating principle that I have already mentioned and that is already there in your instructions or document. One of these isolating principles is also to isolate within the isolated components to see which individual part is performing wrongly"*). Two principles already in this file ("Isolation and investigation" §0 and "AI agents close their own visual loop too" §0) compose into a binding rule for the in-product whiteboard agent: after rendering, the agent must *itself* look at the rendered PNG, attribute any visible defect to the pipeline stage that produced it (Pass 1 narrative? Pass A planning? Pass B placement? wrapper geometry? renderer/ELK layout?), isolate within that stage to the failing sub-component, fix it, and re-render. Concrete consequences:
  ...
  - **Per-stage isolation tooling is half-built and must be finished.** Explicit per-stage attribution + a hook that runs after render do not exist yet. When the in-product agent finds a defect at stage K, the harness must support re-running ONLY stage K with a fix, not regenerating the entire generation chain.
