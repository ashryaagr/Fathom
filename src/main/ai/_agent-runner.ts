 ...
 * loop the lens-explain + decompose call sites in this directory
 * share. Originally extracted to dedup the 7 SDK call sites of the
 * pre-pivot whiteboard pipeline; the whiteboard side now goes
 * through the fathom-whiteboard package, but the lens-explain +
 * decompose paths still use this helper directly.
 *
 * Scope: ONE agent session. Multi-call loops (e.g. lens follow-up
 * turns that share a session) invoke this helper N times — once per
 * call — and keep the outer iteration + any MCP-instance lifecycle
 * in the caller.
 *
 * The helper is INTENTIONALLY narrow:
 *   - It does NOT compute `cwd` (caller's `safeCwd(indexPath)` —
 *     same shape duplicated across files; future dedup target).
 *   - It does NOT resolve the Claude executable path (caller's
 *     `resolveClaudeExecutablePath() ?? undefined`).
 *   - It does NOT manage MCP-instance lifetimes.
 *   - It does NOT log under its own bracket prefix; every site
 *     already logs `[Lens AI]` / etc., and those prefixes are part
 *     of the user-readable log surface. Helper stays silent.
 *
 ...
 *      `result.caughtPostIteratorThrow` so callers can treat as a
 *      soft boundary instead of failing the whole session.
 *
 * Surfaces back to caller via {@link AgentSessionResult}:
 *   - `responseText` — accumulated text deltas; sites that don't
 *     stream-collect ignore this.
 *   - token + cache stats.
...
export interface AgentSessionArgs {
   ...
   * Useful when a caller wants to treat such throws as a soft
   * boundary instead of a hard failure. Other sites omit. */
